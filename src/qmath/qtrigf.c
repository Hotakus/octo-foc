...
void fast_sin_cos_q15(float rad, float *sin_out, float *cos_out) {
    rad = foc_normalize_angle(rad) / PI;

    // rad to q1.15
    int16_t hAngle = (int16_t)(rad * 32768.0f);
    const Trig_Components Local_Components = trig_functions(hAngle);

    // q1.15 to float
    *sin_out = (float)Local_Components.hSin * Q15_TO_FLOAT;
    *cos_out = (float)Local_Components.hCos * Q15_TO_FLOAT;
}
